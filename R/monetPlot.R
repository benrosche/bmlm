...
monetPlot <- function(rmm, parameter, label=NULL, r=2, yaxis=T) {
  ...
  if(is.null(rmm$jags.out)) stop("JAGS output could not be retrieved. Please specify monitor = T when running rmm.")
  if(is.null(label)) label = parameter
  
  # Get mcmclist and posterior stats ------------------------------------------------------------- #
  
  mcmc.list <- coda::as.mcmc(rmm$jags.out)
